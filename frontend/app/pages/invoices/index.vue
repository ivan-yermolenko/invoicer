...
const columns = [
  ...
  { key: 'supplier_name', label: 'Постачальник', alignClass: 'text-left', skeletonClass: 'h-4 w-32' },
  { key: 'gross_amount', label: 'Сума', alignClass: 'text-right', skeletonClass: 'h-4 w-20 ml-auto' },
  { key: 'status', label: 'Статус', alignClass: 'text-center', skeletonClass: 'h-6 w-20 rounded-full mx-auto' },
  { key: 'due_date', label: 'Оплатити до', alignClass: 'text-right', skeletonClass: 'h-4 w-24 ml-auto' }
]
</script>
              ...
              >
                ...
                >
                  ...
                  <template v-else-if="col.key === 'gross_amount'">
                    <span class="font-medium text-slate-900">{{ formatCurrency(invoice.gross_amount, invoice.currency) }}</span>
                  </template>
                  ...
                  <template v-else-if="col.key === 'due_date'">
                    <span class="text-slate-500 group-hover:text-blue-600 transition-colors">{{ formatDate(invoice.due_date) }}</span>
                  </template>
                </td>
              </tr>
            </template>
